...
def validate_repository():
    json_path = r"c:\Users\Home\Desktop\tnpsc_ai\data\questions\polity\salient_features_of_the_indian_constitution_grand_test.json"
    ...
    for idx, q in enumerate(questions, 1):
        q_id = q.get("id")
        assert q_id == f"SF_GT_{idx:03d}", f"Q{idx} ID mismatch: {q_id}"
        ...
        # Check difficulty & question_type
        diff = q["difficulty"]
        assert diff in difficulty_dist, f"Q{idx} invalid difficulty: {diff}"
    ...
    print("Question Type Distribution:", type_dist)
