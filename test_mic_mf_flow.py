...
print("=== VERIFYING MAKING OF INDIAN CONSTITUTION MATCH THE FOLLOWING FLOW ===")
...
print("\n--- 2. Match the Following Question Loading Verification for All 3 Notes Parts ---")
for idx, mic in enumerate(mic_parts, 1):
    meta = get_topic_metadata_by_id("polity", mic)
    qs = load_questions(meta["repository_id"], "match_the_following")
    print(f"Part {idx} ({meta['display_title']}) -> Loaded {len(qs)} match questions from '{meta['repository_id']}_match_the_following.json'")
    assert len(qs) == 25, f"Expected 25 match questions for {mic}, got {len(qs)}"

print("\n--- 3. Checking Question Fields & Syntax for all 25 Questions ---")
with open("data/questions/polity/making_of_indian_constitution_match_the_following.json", encoding="utf-8") as f:
...
required_fields = [
    ...
    "question", "list_1", "list_2", "options", "correct_answer",
...
print(f"[OK] ALL 25 MATCH THE FOLLOWING QUESTIONS VALIDATED SUCCESSFULLY! Sequential IDs: {questions[0]['id']} to {questions[-1]['id']}")
