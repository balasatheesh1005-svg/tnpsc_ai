# -*- coding: utf-8 -*-
"""
End-to-End Practice Engine Simulation for Prime Minister MCQ Datasets
"""
...
def test_pm_practice_ui_simulation():
    print("================================================================================")
    print("🧪 PRIME MINISTER MCQ DATASETS: UI & PRACTICE ENGINE INTERACTIVE SIMULATION")
    print("================================================================================")
    ...
    for mode, expected_count in datasets:
        ...
        # 1. Load questions
        qs = load_questions("polity_prime_minister", mode)
        ...
        success = start_practice_session(
            subject="polity",
            topic_id="polity_prime_minister",
            repository_id="polity_prime_minister",
            display_title=f"Prime Minister - {mode.title()}",
        ...
        for i in range(expected_count):
            ...
            assert isinstance(da, dict) and len(da) == 4, f"Q{i} ({qid}) missing distractor_analysis"
            assert isinstance(wno, dict) and len(wno) == 4, f"Q{i} ({qid}) missing why_not_others"

            # Check tip & high-yield fact
            tip = q.get("tnpsc_expert_tip") or q.get("tnpsc_tip")
            hy = q.get("high_yield_revision_fact") or q.get("high_yield_fact")
            assert tip is not None, f"Q{i} ({qid}) missing tnpsc_expert_tip"
            assert hy is not None, f"Q{i} ({qid}) missing high_yield_revision_fact"
    ...
    print("\n================================================================================")
    if all_passed:
        print("🎉 ALL 8 PRIME MINISTER MCQ DATASETS PASSED UI & PRACTICE ENGINE SIMULATION 100%!")
    else:
...
if __name__ == "__main__":
    test_pm_practice_ui_simulation()
