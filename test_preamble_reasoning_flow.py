# -*- coding: utf-8 -*-
"""
Comprehensive Validation & Practice Engine Flow Verification Suite for Preamble Reasoning (25 MCQs)
"""
...
def run_preamble_reasoning_verification():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - PREAMBLE REASONING (25 MCQs) VERIFICATION SUITE")
    print("================================================================================")
    ...
    with open("data/questions/polity/preamble_assertion_reason.json", "r", encoding="utf-8") as f:
        questions = json.load(f)
    ...
    for idx, q in enumerate(questions, 1):
        q_id = q.get("id")
        ids.append(q_id)
        assert q_id == f"PRE_AR_{idx:03d}", f"❌ Invalid ID pattern: {q_id} at index {idx}"
    ...
    assert avail_p1.get("assertion_reason") == True, "❌ Assertion Reason repo should be available for Preamble Part 1!"
    qs_p1 = load_questions("polity", "polity_preamble_part_1", "assertion_reason")
    assert len(qs_p1) == 25, f"❌ Expected 25 questions for Part 1, got {len(qs_p1)}"
    print(f"   ✅ Preamble Part 1 loads {len(qs_p1)} Reasoning questions successfully")

    # Part 2 check
    avail_p2 = check_repository_availability("polity", "polity_preamble_part_2")
    print(f"   - Preamble Part 2 Availability: {avail_p2}")
    assert avail_p2.get("assertion_reason") == True, "❌ Assertion Reason repo should be available for Preamble Part 2!"
    qs_p2 = load_questions("polity", "polity_preamble_part_2", "assertion_reason")
    assert len(qs_p2) == 25, f"❌ Expected 25 questions for Part 2, got {len(qs_p2)}"
    print(f"   ✅ Preamble Part 2 loads {len(qs_p2)} Reasoning questions successfully")

    # Common repository identity check
    assert qs_p1[0]["id"] == qs_p2[0]["id"] == "PRE_AR_001", "❌ Part 1 and Part 2 must load identical repository!"
    assert qs_p1[-1]["id"] == qs_p2[-1]["id"] == "PRE_AR_025", "❌ Part 1 and Part 2 end ID mismatch!"
    print("   ✅ Common Topic-Level Repository Shared Load PASSED")

    # Direct repository_id call checks for both 'assertion_reason' and 'reasoning'
    qs_direct_ar = load_questions("polity_preamble", "assertion_reason")
    qs_direct_r = load_questions("polity_preamble", "reasoning")
    assert len(qs_direct_ar) == 25, "❌ Direct load_questions('polity_preamble', 'assertion_reason') failed!"
    assert len(qs_direct_r) == 25, "❌ Direct load_questions('polity_preamble', 'reasoning') failed!"
    print(f"   ✅ Direct load_questions('polity_preamble', 'assertion_reason') & ('reasoning') PASSED (25 questions each)")

    # -------------------------------------------------------------------------
    # 5. Question Engine Adapter Normalization Check
    # -------------------------------------------------------------------------
    print("\n[STEP 5] Universal Question Engine Adapter Normalization")
    for idx, q_raw in enumerate(questions):
        ...
        assert norm_q.id == f"PRE_AR_{idx+1:03d}"
    ...
    regression_checks = [
        ("polity", "polity_historical_background_part1", "assertion_reason", 25),
        ("polity", "polity_making_of_indian_constitution_part_1", "assertion_reason", 25),
        ("polity", "polity_salient_features_of_the_indian_constitution_part_1", "assertion_reason", 25),
...
if __name__ == "__main__":
    run_preamble_reasoning_verification()
