# -*- coding: utf-8 -*-
"""
Verification Suite for Fundamental Rights Hard 50 MCQs
"""
...
def run_fr_hard_mcq_verification():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - FUNDAMENTAL RIGHTS HARD 50 MCQs VERIFICATION SUITE")
    print("================================================================================")

    # 1. File Existence & JSON Parsing
    file_path = "data/questions/polity/fundamental_rights_hard.json"
    ...
    assert q_ids[0] == "FR_H_001" and q_ids[-1] == "FR_H_050", "❌ Incorrect ID range"
    print("   ✅ All 50 IDs are unique and follow FR_H_001 to FR_H_050 convention.")
    ...
    for idx, q in enumerate(data, start=1):
        ...
        assert q.get("difficulty") == "Hard", f"❌ [{qid}] Invalid difficulty"
    ...
    assert type_dist.get("Multi-Statement", 0) >= 8, "❌ Missing Multi-Statement questions"
    assert type_dist.get("Advanced Conceptual", 0) >= 8, "❌ Missing Advanced Conceptual questions"
    assert type_dist.get("Case-law Based", 0) >= 5, "❌ Missing Case-law Based questions"
    assert type_dist.get("Constitutional Application", 0) >= 6, "❌ Missing Constitutional Application questions"
    assert type_dist.get("High-level TNPSC Trap", 0) >= 5, "❌ Missing High-level TNPSC Trap questions"
    assert type_dist.get("Amendment / Basic Structure", 0) >= 4, "❌ Missing Amendment / Basic Structure questions"
    ...
    q_main = load_questions("polity_fundamental_rights", "hard")
    assert len(q_main) == 50, f"❌ Loader failed to load 50 questions for repo_id 'polity_fundamental_rights', got {len(q_main)}"
    print("   ✅ load_questions('polity_fundamental_rights', 'hard') -> Loaded 50 questions!")
    ...
    for p_topic in parts:
        p_questions = load_questions("polity", p_topic, "hard")
        assert len(p_questions) == 50, f"❌ Failed loading Hard repo for {p_topic}"
        assert p_questions[0]["id"] == "FR_H_001", f"❌ Incorrect first question ID for {p_topic}"
        print(f"   [OK] {p_topic} -> Practice -> Hard loads the SAME 50 questions!")

    # 6. Regression Check on Existing Hard Repositories
    print("\n[STEP 6] Regression Check on Existing Hard Repositories")
    reg_repos = [
        ("polity_historical_background", "hard"),
        ("polity_making_of_indian_constitution", "hard"),
        ("polity_preamble", "hard"),
        ("polity_salient_features_of_the_indian_constitution", "hard")
    ]
    ...
    print("\n================================================================================")
    print("🎉 ALL 6 VERIFICATION STEPS PASSED WITH 100% SUCCESS FOR HARD MCQs!")
    print("================================================================================")

if __name__ == "__main__":
    run_fr_hard_mcq_verification()
