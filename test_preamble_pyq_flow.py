# -*- coding: utf-8 -*-
"""
Comprehensive Validation & Practice Engine Flow Verification Suite for Preamble PYQ Practice (50 MCQs)
"""
...
def run_preamble_pyq_verification():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - PREAMBLE PYQ PRACTICE (50 MCQs) VERIFICATION SUITE")
    print("================================================================================")
    ...
    with open("data/questions/polity/preamble_pyq.json", "r", encoding="utf-8") as f:
        questions = json.load(f)
    ...
    for idx, q in enumerate(questions, 1):
        q_id = q.get("id")
        ids.append(q_id)
        assert q_id == f"PRE_PYQ_{idx:03d}", f"❌ Invalid ID pattern: {q_id} at index {idx}"
    ...
    for k in sorted(counts.keys()):
        print(f"   - Option {k}: {counts[k]} ({counts[k]/50*100:.1f}%)")
    assert all(counts[k] >= 10 for k in ["A", "B", "C", "D"]), "❌ Imbalanced answer distribution!"
    ...
    assert avail_p1.get("pyq") == True, "❌ PYQ repo should be available for Preamble Part 1!"
    qs_p1 = load_questions("polity", "polity_preamble_part_1", "pyq")
    assert len(qs_p1) == 50, f"❌ Expected 50 questions for Part 1, got {len(qs_p1)}"
    print(f"   ✅ Preamble Part 1 loads {len(qs_p1)} PYQ Practice questions successfully")

    # Part 2 check
    avail_p2 = check_repository_availability("polity", "polity_preamble_part_2")
    print(f"   - Preamble Part 2 Availability: {avail_p2}")
    assert avail_p2.get("pyq") == True, "❌ PYQ repo should be available for Preamble Part 2!"
    qs_p2 = load_questions("polity", "polity_preamble_part_2", "pyq")
    assert len(qs_p2) == 50, f"❌ Expected 50 questions for Part 2, got {len(qs_p2)}"
    print(f"   ✅ Preamble Part 2 loads {len(qs_p2)} PYQ Practice questions successfully")

    # Common repository identity check
    assert qs_p1[0]["id"] == qs_p2[0]["id"] == "PRE_PYQ_001", "❌ Part 1 and Part 2 must load identical repository!"
    assert qs_p1[-1]["id"] == qs_p2[-1]["id"] == "PRE_PYQ_050", "❌ Part 1 and Part 2 end ID mismatch!"
    print("   ✅ Common Topic-Level Repository Shared Load PASSED")

    # Direct repository_id call check
    qs_direct_pyq = load_questions("polity_preamble", "pyq")
    assert len(qs_direct_pyq) == 50, "❌ Direct load_questions('polity_preamble', 'pyq') failed!"
    print(f"   ✅ Direct load_questions('polity_preamble', 'pyq') PASSED ({len(qs_direct_pyq)} questions)")
    ...
    for idx, q_raw in enumerate(questions):
        ...
        assert norm_q.id == f"PRE_PYQ_{idx+1:03d}"
    ...
    print("   ✅ Universal Question Adapter normalized all 50 questions successfully")
    ...
    regression_checks = [
        ("polity", "polity_historical_background_part1", "pyq", 50),
        ("polity", "polity_making_of_indian_constitution_part_1", "pyq", 50),
        ("polity", "polity_salient_features_of_the_indian_constitution_part_1", "pyq", 40),
