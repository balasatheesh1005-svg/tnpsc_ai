...
def test_preamble_easy_suite():
    print("==================================================")
    print("RUNNING PREAMBLE EASY 50 PRACTICE VERIFICATION SUITE")
    print("==================================================")

    # 1. File existence & JSON parsing
    file_path = "data/questions/polity/preamble_easy.json"
    ...
    print("   [OK] All 50 questions pass dual-schema validation")
    ...
    print("\n4. Testing Topic Navigation - Preamble Part 1:")
    avail_p1 = check_repository_availability("polity", "polity_preamble_part_1")
    print(f"   - polity_preamble_part_1 availability: {avail_p1}")
    assert avail_p1.get("easy") == True, "Easy repository should be available for Preamble Part 1!"

    qs_p1 = load_questions("polity", "polity_preamble_part_1", "easy")
    ...
    print("\n5. Testing Topic Navigation - Preamble Part 2:")
    avail_p2 = check_repository_availability("polity", "polity_preamble_part_2")
    print(f"   - polity_preamble_part_2 availability: {avail_p2}")
    assert avail_p2.get("easy") == True, "Easy repository should be available for Preamble Part 2!"

    qs_p2 = load_questions("polity", "polity_preamble_part_2", "easy")
    print(f"   - Loaded questions for Part 2: {len(qs_p2)}")
    assert len(qs_p2) == 50, f"Expected 50 questions for Part 2, got {len(qs_p2)}"
    assert qs_p1[0]["id"] == qs_p2[0]["id"], "Part 1 and Part 2 should load the common Preamble Easy repository!"
    print("   [OK] Part 2 Navigation & Shared Repository Load PASSED")

    # 6. Regression Testing Other Topics
    print("\n6. Regression Verification of Existing Easy Repositories:")
    other_topics = [
        ("polity", "polity_historical_background_part1"),
        ("polity", "polity_making_of_indian_constitution_part_1"),
        ("polity", "polity_salient_features_of_the_indian_constitution_part_1")
    ]
    for subj, top_id in other_topics:
        avail = check_repository_availability(subj, top_id)
        assert avail.get("easy") == True, f"Easy repo broken for {top_id}"
        loaded = load_questions(subj, top_id, "easy")
        assert len(loaded) >= 50, f"Question count insufficient for {top_id}"
        print(f"   [OK] {top_id} loads {len(loaded)} Easy questions cleanly.")

    print("\n==================================================")
    print("ALL PREAMBLE EASY 50 PRACTICE VERIFICATION CHECKS PASSED SUCCESSFULLY!")
    print("==================================================")

if __name__ == "__main__":
    test_preamble_easy_suite()
