...
print("=== VERIFYING MAKING OF INDIAN CONSTITUTION REASONING / ASSERTION & REASON FLOW ===")
...
print("\n--- 2. Assertion & Reason Question Loading Verification for All 3 Notes Parts ---")
for idx, mic in enumerate(mic_parts, 1):
    meta = get_topic_metadata_by_id("polity", mic)
    qs = load_questions(meta["repository_id"], "assertion_reason")
    print(f"Part {idx} ({meta['display_title']}) -> Loaded {len(qs)} reasoning questions from '{meta['repository_id']}_assertion_reason.json'")
    assert len(qs) == 25, f"Expected 25 reasoning questions for {mic}, got {len(qs)}"

print("\n--- 3. Checking Question Fields & Syntax for all 25 Questions ---")
with open("data/questions/polity/making_of_indian_constitution_assertion_reason.json", encoding="utf-8") as f:
...
print(f"[OK] ALL 25 REASONING QUESTIONS VALIDATED SUCCESSFULLY! Sequential IDs: {questions[0]['id']} to {questions[-1]['id']}")
