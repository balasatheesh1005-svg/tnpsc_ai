...
def test_preamble_part1():
    print("==================================================")
    print("RUNNING PREAMBLE PART 1 NOTES VERIFICATION SUITE")
    print("==================================================")
    ...
    assert meta["topic_id"] == "polity_preamble_part_1", "topic_id mismatch!"
    ...
    assert avail.get("notes") == True, "Notes should be available for Preamble Part 1!"
    print("   [OK] Availability check PASSED")

    # 3. Load Notes JSON Payload
    file_path = "data/notes/polity/preamble_part_1.json"
    assert os.path.exists(file_path), f"File {file_path} does not exist!"

    data = load_note(file_path)
    assert data is not None, "Failed to load preamble_part_1.json payload!"
    ...
    assert "keywords" in data and len(data["keywords"]) > 5, "Missing or insufficient keywords"
    ...
    assert "tnpsc_traps" in data, "Missing tnpsc_traps"
    assert "revision_cards" in data and len(data["revision_cards"]) >= 5, "Missing or insufficient revision cards"

    content_str = json.dumps(data, ensure_ascii=False)

    scope_checks = [
        ("Meaning of Preamble", "Preamble"),
        ...
        ("Dignity of Individual", "Dignity"),
        ("Unity and Integrity", "Integrity"),
    ...
    # 6. Verify Existing Topics Compatibility
    ...
    print("\n==================================================")
    print("ALL PREAMBLE PART 1 NOTES VERIFICATION CHECKS PASSED SUCCESSFULLY!")
    print("==================================================")
