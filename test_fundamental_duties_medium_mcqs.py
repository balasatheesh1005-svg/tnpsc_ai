# -*- coding: utf-8 -*-
"""
Validation & Verification Test Suite for Fundamental Duties Medium 50 MCQs
"""
...
def test_fundamental_duties_medium_mcqs_flow():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - FUNDAMENTAL DUTIES MEDIUM 50 MCQs VERIFICATION SUITE")
    print("================================================================================")

    # 1. File existence & JSON parsing
    q_path = "data/questions/polity/fundamental_duties_medium.json"
    ...
    for idx, q in enumerate(questions, 1):
        ...
        # Difficulty & Topic
        assert q["difficulty"] == "Medium", f"❌ Question {idx} invalid difficulty: {q['difficulty']}"
    ...
    assert key_counts["A"] >= 8 and key_counts["B"] >= 8 and key_counts["C"] >= 8 and key_counts["D"] >= 8, f"❌ Unbalanced answer distribution: {key_counts}"
    ...
    for part_num in [1, 2, 3]:
        ...
        loaded_qs = load_questions("polity", t_id, "medium")
        assert loaded_qs is not None, f"❌ Failed to load medium questions for {t_id}"
        assert len(loaded_qs) == 50, f"❌ Loaded {len(loaded_qs)} questions for {t_id}, expected 50!"
        assert loaded_qs[0]["id"] == "FD_M_001", f"❌ Mismatched repository loaded for {t_id}!"
        print(f"   - Part {part_num} (`{t_id}`) -> Loaded 50 Medium questions from common repository -> PASSED")

    print("\n================================================================================")
    print("🏆 ALL VERIFICATION CHECKS PASSED FOR FUNDAMENTAL DUTIES MEDIUM 50 MCQs!")
    print("================================================================================")

if __name__ == "__main__":
    test_fundamental_duties_medium_mcqs_flow()
