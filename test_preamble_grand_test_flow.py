# -*- coding: utf-8 -*-
"""
Comprehensive Validation & Practice Engine Flow Verification Suite for Preamble Grand Test (100 MCQs)
"""
...
def run_preamble_grand_test_verification():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - PREAMBLE GRAND TEST (100 MCQs) VERIFICATION SUITE")
    print("================================================================================")
    ...
    for idx, q in enumerate(questions, 1):
        q_id = q.get("id")
        ids.append(q_id)
        assert q_id == f"PRE_GT_{idx:03d}", f"❌ Invalid ID pattern: {q_id} at index {idx}"
    ...
    # -------------------------------------------------------------------------
    # 5. Navigation & Availability Verification (Part 1 & Part 2 Common Loading)
    # -------------------------------------------------------------------------
    ...
    assert avail_p1.get("grand_test") == True, "❌ Grand Test repo should be available for Preamble Part 1!"
    qs_p1 = load_questions("polity", "polity_preamble_part_1", "grand_test")
    assert len(qs_p1) == 100, f"❌ Expected 100 questions for Part 1, got {len(qs_p1)}"
    print(f"   ✅ Preamble Part 1 loads {len(qs_p1)} Grand Test questions successfully")

    # Part 2 check
    avail_p2 = check_repository_availability("polity", "polity_preamble_part_2")
    print(f"   - Preamble Part 2 Availability: {avail_p2}")
    assert avail_p2.get("grand_test") == True, "❌ Grand Test repo should be available for Preamble Part 2!"
    qs_p2 = load_questions("polity", "polity_preamble_part_2", "grand_test")
    assert len(qs_p2) == 100, f"❌ Expected 100 questions for Part 2, got {len(qs_p2)}"
    print(f"   ✅ Preamble Part 2 loads {len(qs_p2)} Grand Test questions successfully")

    # Common repository identity check
    assert qs_p1[0]["id"] == qs_p2[0]["id"] == "PRE_GT_001", "❌ Part 1 and Part 2 must load identical repository!"
    assert qs_p1[-1]["id"] == qs_p2[-1]["id"] == "PRE_GT_100", "❌ Part 1 and Part 2 end ID mismatch!"
    print("   ✅ Common Topic-Level Repository Shared Load PASSED")

    # Direct repository_id call check
    qs_direct_gt = load_questions("polity_preamble", "grand_test")
    assert len(qs_direct_gt) == 100, "❌ Direct load_questions('polity_preamble', 'grand_test') failed!"
    print(f"   ✅ Direct load_questions('polity_preamble', 'grand_test') PASSED ({len(qs_direct_gt)} questions)")

    # -------------------------------------------------------------------------
    # 6. Question Engine Adapter Normalization Check
    # -------------------------------------------------------------------------
    print("\n[STEP 6] Universal Question Engine Adapter Normalization")
    for idx, q_raw in enumerate(questions):
        ...
        assert norm_q.id == f"PRE_GT_{idx+1:03d}"
    ...
    print("   ✅ Universal Question Adapter normalized all 100 questions successfully")

    # -------------------------------------------------------------------------
    # 7. Regression Testing Across Existing Repositories
    # -------------------------------------------------------------------------
    print("\n[STEP 7] Regression Verification of Existing Repositories")
    regression_checks = [
        ("polity", "polity_historical_background_part1", "grand_test", 100),
        ("polity", "polity_making_of_indian_constitution_part_1", "grand_test", 100),
        ("polity", "polity_salient_features_of_the_indian_constitution_part_1", "grand_test", 100),
        ...
        ("polity", "polity_preamble_part_1", "chronology", 25),
        ("polity", "polity_preamble_part_1", "assertion_reason", 25),
        ("polity", "polity_preamble_part_1", "pyq", 50),
    ]
    ...
    print("\n================================================================================")
    print("🎉 ALL 7 VERIFICATION STEPS PASSED WITH 100% SUCCESS!")
    print("================================================================================")

if __name__ == "__main__":
    run_preamble_grand_test_verification()
