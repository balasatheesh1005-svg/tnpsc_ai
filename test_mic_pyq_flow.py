...
print("=== VERIFYING MAKING OF INDIAN CONSTITUTION PYQ PRACTICE FLOW ===")
...
print("\n--- 2. PYQ Question Loading Verification for All 3 Notes Parts ---")
for idx, mic in enumerate(mic_parts, 1):
    meta = get_topic_metadata_by_id("polity", mic)
    qs = load_questions(meta["repository_id"], "pyq")
    print(f"Part {idx} ({meta['display_title']}) -> Loaded {len(qs)} PYQ questions from '{meta['repository_id']}_pyq.json'")
    assert len(qs) == 50, f"Expected 50 PYQ questions for {mic}, got {len(qs)}"

print("\n--- 3. Checking Question Fields & Syntax for all 50 Questions ---")
with open("data/questions/polity/making_of_indian_constitution_pyq.json", encoding="utf-8") as f:
...
print(f"[OK] ALL 50 PYQ PRACTICE QUESTIONS VALIDATED SUCCESSFULLY! Sequential IDs: {questions[0]['id']} to {questions[-1]['id']}")
