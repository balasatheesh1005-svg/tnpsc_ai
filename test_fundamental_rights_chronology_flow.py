# -*- coding: utf-8 -*-
"""
Verification Suite for Fundamental Rights Chronology MCQs Repository
"""
...
def run_verification():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - FUNDAMENTAL RIGHTS CHRONOLOGY MCQS VERIFICATION SUITE")
    print("================================================================================")

    # 1. File Existence & JSON parsing
    target_file = "data/questions/polity/fundamental_rights_chronology.json"
    ...
    print("\n[STEP 2] Auditing Chronology Schema Fields for all 25 Questions")
    required_fields = [
    ...
    for idx, q in enumerate(questions, 1):
        ...
        # Check IDs
        expected_id = f"FR_CHRONO_{idx:03d}"
        assert q["id"] == expected_id, f"❌ Question {idx} ID mismatch: expected {expected_id}, got {q['id']}"

        # Check events & options structure
        assert len(q["events"]) == 4, f"❌ Question {idx} events count is not 4"
    ...
    print("   ✅ All 25 questions contain valid Chronology structures and required fields.")
    ...
    for topic_id in fr_topics:
        ...
        print(f"   - {meta['display_title']} -> chronology availability: {avail.get('chronology')}")
        assert avail.get("chronology") == True, f"❌ chronology availability is False for {topic_id}"

        # Test loading questions
        loaded_qs = load_questions(meta["repository_id"], "chronology")
        assert len(loaded_qs) == 25, f"❌ Failed to load 25 chronology questions for {topic_id}, loaded {len(loaded_qs)}"
        print(f"   [OK] {meta['display_title']} -> Loaded {len(loaded_qs)} Chronology questions cleanly.")

    print("   ✅ Practice Engine Integration PASSED for Part 1, Part 2, and Part 3!")

    print("\n================================================================================")
    print("🎉 ALL VERIFICATION STEPS PASSED WITH 100% SUCCESS FOR CHRONOLOGY MCQS!")
    print("================================================================================")
