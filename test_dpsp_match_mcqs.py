# -*- coding: utf-8 -*-
"""
Validation & Verification Test Suite for DPSP Match 25 MCQs
"""
...
def test_dpsp_match_mcqs_flow():
    print("================================================================================")
    print("🚀 RUNNING TNPSC NOVA AI - DPSP MATCH THE FOLLOWING 25 MCQs VERIFICATION SUITE")
    print("================================================================================")

    # 1. File existence & JSON parsing
    q_path = "data/questions/polity/directive_principles_match.json"
    ...
    print("   ✅ Question file exists and contains exactly 25 questions.")
    ...
    for idx, q in enumerate(questions, 1):
        # Check required keys
        req_keys = ["id", "subject", "topic", "difficulty", "question_type", "question", "list_1", "list_2", "options", "correct_answer", "explanation", "why_not_others", "tnpsc_tip"]
        for k in req_keys:
            assert k in q, f"❌ Question {idx} ({q.get('id')}) missing key: {k}"

        # Topic & Question Type
        assert q["topic"] == "Directive Principles of State Policy", f"❌ Question {idx} invalid topic: {q['topic']}"
        assert q["question_type"] == "Match the Following", f"❌ Question {idx} invalid question_type: {q['question_type']}"
        ...
        l1 = q["list_1"]
        l2 = q["list_2"]
        assert len(l1) == 4, f"❌ Question {idx} list_1 does not have 4 items!"
        assert len(l2) == 4, f"❌ Question {idx} list_2 does not have 4 items!"
    ...
    print("   ✅ All 25 question objects passed schema and non-empty bilingual audit.")

    # 3. Answer Key Distribution
    print(f"\n[STEP 3] Verifying Answer Key Distribution: {key_counts}")
    assert key_counts["A"] >= 5 and key_counts["B"] >= 5 and key_counts["C"] >= 5 and key_counts["D"] >= 5, "❌ Unbalanced answer distribution!"
    assert key_counts["A"] < 25, "❌ Correct answer bug detected: all correct answers are A!"
    print("   ✅ Answer key distribution balanced (A=6, B=6, C=6, D=7).")

    # 4. Cross-part Navigation Loader Resolution Test
    print("\n[STEP 4] Testing Cross-part Question Loader Resolution")
    for part_num in [1, 2, 3]:
        t_id = f"polity_directive_principles_part_{part_num}"
        loaded_qs = load_questions("polity", t_id, "match")
        assert loaded_qs is not None, f"❌ Failed to load match questions for {t_id}"
        assert len(loaded_qs) == 25, f"❌ Loaded {len(loaded_qs)} questions for {t_id}, expected 25!"
        assert loaded_qs[0]["id"] == "DPSP_MTH_001", f"❌ Mismatched repository loaded for {t_id}!"
        print(f"   - Part {part_num} (`{t_id}`) -> Loaded 25 questions from common repository -> PASSED")

    print("\n================================================================================")
    print("🏆 ALL VERIFICATION CHECKS PASSED FOR DPSP MATCH THE FOLLOWING 25 MCQs!")
    print("================================================================================")

if __name__ == "__main__":
    test_dpsp_match_mcqs_flow()
